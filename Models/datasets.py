    ...
    def __getitem__(self, idx):
        ...
        if 'image' in self.sources:
            ...
            for i in range(min(self.max_views,len(img_files))):
                ...
                img_file = '/Users/chenyusha/PycharmProjects/Medical-Report-Generation/data/image/NLMCXR_png/' + img_files[i] + '.png'
                img = Image.open(img_file).convert('RGB')
                imgs.append(self.transform(img).unsqueeze(0)) # (1,C,W,H)
        ...
        for i in range(len(self.targets)):
            if self.targets[i] == 'label':
                targets.append(np.concatenate([np.array(self.file_labels[file_name]), np_labels]))  # np.concatenate矩阵合并
            if self.targets[i] == 'caption':
                targets.append(target_info)
            if self.targets[i] == 'caption_length':
                targets.append(min(len(encoded_target_info), self.max_len))
...
class TestImageDataset(data.Dataset):
    def __init__(self, sources=['image'], input_size=(256,256), random_transform=True, max_len=1000):
        self.sources = sources  # Choose which section as input
        self.max_len = max_len
        if random_transform:
            self.transform = transforms.Compose([
    ...
    def __getitem__(self, idx):
        ...
        for i in range(min(2, 1)):
            # img_file = self.dir + 'images/' + img_files[i] + '.png'
            img_file = 'img/test/CXR1_1_IM-0001-3001.png'
            img = Image.open(img_file).convert('RGB')
            imgs.append(self.transform(img).unsqueeze(0))  # (1,C,W,H) unsqueeze()这个函数主要是对数据维度进行扩充。给指定位置加上维数为1的维度
            vpos.append(1)  # We do not know what view position of the image is, so just let it be 1
        ...
        imgs = torch.cat(imgs, dim=0)  # (V,C,W,H) 上方的unsqueeze(0)是为了这里能将2个tensor连接
        vpos = np.array(vpos, dtype=np.int64)  # (V)
