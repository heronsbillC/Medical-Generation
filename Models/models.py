...
class PositionalEncoding(nn.Module):
    """\n    Positional Encoding module used in the Transformer model.\n    """

    def __init__(self, d_model, dropout=0.1, max_len=5000):
        """\n        Args:\n            d_model (int): The dimensionality of the model's hidden states.\n            dropout (float, optional): The dropout probability to apply to the output tensor. Default: 0.1.\n            max_len (int, optional): The maximum length of the input sequence. Default: 5000.\n        """
        super().__init__()
    ...
    def forward(self, x):
        """\n        Args:\n            x (torch.Tensor): An input tensor of shape (batch_size, seq_len, d_model).\n\n        Returns:\n            torch.Tensor: The output tensor of shape (batch_size, seq_len, d_model) with positional encoding applied.\n        """
...
class CNN(nn.Module):
    def __init__(self, model, model_type='resnet'):
        ...
        if 'res' in model_type.lower():  # resnet, resnet-50, resnest-50, ...
            ...
            # 是一个池化层，可以将它的输出作为模型的最终输出。这意味着模型将对输入数据提取特征，并将这些特征的平均值作为最终输出。
            ...
            # 一个自适应平均池化层，该层将通过对每个特征映射的值进行平均来输出一个1x1的张量。这个层的输入大小不是固定的，因为它将自动适应输入张量的大小。通常，在卷积神经网络的最后一层中使用这个层，将输出张量的空间维度减少到每个特征映射的单个值。
...
class MVCNN(nn.Module):
    ...
    def forward(self, input):
        ...
        pos = input[1] # (B,V)
        B,V,C,W,H = img.shape #H-高，W-宽，C-通道数（色彩空间如RGB），
...
# --- Main Moduldes ---
# 分类模块读取多个胸部X射线图像，并通过多视图图像编码器提取全局视觉特征表示。然后将它们分解为多个低维视觉嵌入。
# 同时，文本编码器读取临床文档，包括医生指示，并将内容汇总为文本摘要嵌入
class Classifier(nn.Module):
    ...
    def forward(self, img=None, txt=None, lbl=None, txt_embed=None, pad_mask=None, pad_id=3, threshold=0.5, get_embed=False, get_txt_att=False):
        ...
        if img != None and (txt != None or txt_embed != None):
            ...
            # topic_embed learned in training through the medical report generation pipeline
        ...
        # Classifier output
        # state_embed learned via the classification of final_embed
...
class Generator(nn.Module):
    ...
    def generate_square_identity_mask(self, sz):
        mask = (torch.eye(sz) == 1) #生成对角线全1，其余部分全0的二维数组
        mask = mask.float().masked_fill(mask == 0, float('-inf')).masked_fill(mask == 1, float(0.0))
        return mask 

    def generate_square_mask(self, sz):
        mask = (torch.ones(sz,sz) == 1) #一个用标量值 1 填充的张量，其形状由变量参数 size 定义
        mask = mask.float().masked_fill(mask == 0, float('-inf')).masked_fill(mask == 1, float(0.0))
        return mask
